...
export default class DoneAssigments extends React.Component {
  ...
  componentDidMount() {
    ...
    //update state!
    fetch(
      "http://localhost:8080/api/doneassignments?managerID=1&requestedDate=2001-01-01&pageNumber=1&limit=10"
    )
      .then(response => response.json())
      .then(assigments =>{
        this.setState({ assigments }, () => console.log(this.state.assigments))
      });
